...
function jqAjaxPromise(param, progressFun = () => { }, origin = "web") {
  ...
  request_header['Request-Language'] = 'ko';
...
const getinsurValue = () => {
  ...
  if (url == "/face-swap.html") {
    insurValue = "ko_vidq_faceswap";
  } else if (url == "/video-translator.html") {
    insurValue = "ko_vidq_videotranslate";
  } else if (url == "/") {
    // insurValue = "ko_home";
  } else if (url == "/about.html") {
    // insurValue = "ko_about";
  } else if (url == "/contact.html") {
    // insurValue = "ko_contact";
  } else if (url == "/privacy.html") {
    insurValue = "ko_privacy";
  } else if (url == "/terms.html") {
    insurValue = "ko_terms";
  } else if (url == "/ai-attractiveness-test.html") {
    insurValue = "ko_vidq_attracttest";
  }else if (url == "/multiple-face-swap.html") {
    insurValue = "ko_vidq_mulfaceswap";
  }

  if (isSeo()) {
    insurValue = insurValue.replace("ko", "koseo");
  }
  return insurValue;
};
  ...
  fetchGet("ai/public/get-country-type")
    .then((res) => {
      ...
      if ((countryType === "T1" || countryType === "T2") && judgeClient() === 'ios' && document.location.pathname !== '/pricing.html') {
        $('.fix-footer').css("bottom", '1.18rem');
        $("#iosFooter").show()
        $("#ios_footer_btn").click(function () {
          gtag("event", "ko_faceappclick_fcios");
          window.location.href = "https://apps.apple.com/app/apple-store/id6497986083?pt=127019031&ct=from_kr_vidq&mt=8";
        });
      }
    }) 
    .catch((err) => {
