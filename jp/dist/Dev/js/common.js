...
function jqAjaxPromise(param, progressFun = () => { }, origin = "web") {
  ...
  request_header['Request-Language'] = 'jp';
...
const getinsurValue = () => {
  ...
  if (url == "/face-swap.html") {
    insurValue = "jp_vidq_faceswap";
  } else if (url == "/video-translator.html") {
    insurValue = "jp_vidq_videotranslate";
  } else if (url == "/") {
    // insurValue = "jp_home";
  } else if (url == "/about.html") {
    // insurValue = "jp_about";
  } else if (url == "/contact.html") {
    // insurValue = "jp_contact";
  } else if (url == "/privacy.html") {
    insurValue = "jp_privacy";
  } else if (url == "/terms.html") {
    insurValue = "jp_terms";
  } else if (url == "/ai-attractiveness-test.html") {
    insurValue = "jp_vidq_attracttest";
  }else if (url == "/multiple-face-swap.html") {
    insurValue = "jp_vidq_mulfaceswap";
  }

  if (isSeo()) {
    insurValue = insurValue.replace("jp", "jpseo");
  }
  return insurValue;
};
    ...
    .then((res) => {
      ...
      if ((countryType === "T1" || countryType === "T2") && judgeClient() === 'ios' && document.location.pathname !== '/pricing.html') {
        ...
        $("#ios_footer_btn").click(function () {
          gtag("event", "it_faceappclick_fcios");
          window.location.href = "https://apps.apple.com/app/apple-store/id6497986083?pt=127019031&ct=from_jp_vidq&mt=8";
        });
      }
    }) 
    .catch((err) => {
