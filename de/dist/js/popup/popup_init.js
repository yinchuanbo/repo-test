...
function ToolTip(params) {
  ...
  let length = document.getElementsByTagName("popup-lite").length;
  type === "progress" ? (close = 1) : (close = 0);

  if (length > 0) {
    let popup = document.getElementById("website_popup");
...
function toBindEvent(params) {
  ...
  let popupBox = document.querySelector(".website_popup");
  let popup = document.getElementById("website_popup");
...
function tooltipSupply(params) {
  let {
    ...
    url = "",
    name = "",
    clickHereClassname = "#website_popup .downloadRepeat",
    sucessCallback = ()=>{},
    failedCallback = ()=>{},
    NoExistCallback = ()=>{},
  } = params;
