...
function jqAjaxPromise(param, progressFun = () => { }, origin = "web") {
  ...
  request_header['Request-Language'] = 'tw';
...
const getinsurValue = () => {
  ...
  if (url == "/face-swap.html") {
    insurValue = "tw_vidq_faceswap";
  } else if (url == "/video-translator.html") {
    insurValue = "tw_vidq_videotranslate";
  } else if (url == "/") {
    // insurValue = "tw_home";
  } else if (url == "/about.html") {
    // insurValue = "tw_about";
  } else if (url == "/contact.html") {
    // insurValue = "tw_contact";
  } else if (url == "/privacy.html") {
    insurValue = "tw_privacy";
  } else if (url == "/terms.html") {
    insurValue = "tw_terms";
  } else if (url == "/ai-attractiveness-test.html") {
    insurValue = "tw_vidq_attracttest";
  }else if (url == "/multiple-face-swap.html") {
    insurValue = "tw_vidq_mulfaceswap";
  }

  if (isSeo()) {
    insurValue = insurValue.replace("tw", "twseo");
  }
  return insurValue;
};
    ...
    .then((res) => {
      ...
      if ((countryType === "T1" || countryType === "T2") && judgeClient() === 'ios' && document.location.pathname !== '/pricing.html') {
        ...
        $("#ios_footer_btn").click(function () {
          gtag("event", "tw_faceappclick_fcios");
          window.location.href = "https://apps.apple.com/app/apple-store/id6497986083?pt=127019031&ct=from_tw_vidq&mt=8";
        });
      }
    }) 
    .catch((err) => {
