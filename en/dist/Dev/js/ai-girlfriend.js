...
class AiGirlFriend {
  constructor() {
    ...
    $(window).bind("beforeunload", function () {
      console.log(that.tab, "tab");
      // 1.6
      if (aiChatting.isChat) {
        sessionStorage.setItem("repeatData", JSON.stringify({ path: "chat", data: aiChatting.aiData }));
      } else {
        let historyParam = "home";
        if (that.tab == "my") {
          historyParam = "my-characters";
        } else if (that.tab == "voice") {
          historyParam = "voice-pool";
        }
        sessionStorage.setItem("repeatData", JSON.stringify({ path: historyParam, data: {} }));
      }

      that.access_token = getCookie("access_token") ? getCookie("access_token") : "";
  ...
  data() {
    return {
      ...
      deleteRecArr: [],
      isClickSelectAllRec: false, //是否打开select ALl
    };
  }
  async init(isMy) {
    if (isMy) {
            ...
            item.like_num
                                      ...
                                      ${
                                        item.status == 1
                                          ? `<span class="num">${formatNumber(item.like_num)}</span>
                                  <div class="like_num">${
                                    item.like_num > 1 ? jsonAiGirlFriend.likes : jsonAiGirlFriend.like
                                  }</div>`
                                          : ``
                                      }
  ...
  async GetRecentChatting() {
    try {
      const res = await fetchGet(`chat/user/get-role-list?type=recent_chatting&page_size=50`);
      if (res.code === 200) {
        let data = res.data.list;
        ...
        if (data.length > 0) {
          ...
          $(".RecentChatting").css("display", "block");
          let initHtml = "";
          data.forEach((item, index) => {
            initHtml += `<div class="Recent_item" data-index="${index}" data-id="${item.chat_id}">
                          ...
                          <div class="name">${item.name}</div>
                          <div class="checkBox_recent" style="display: ${isEdit ? "flex" : "none"};">
            </div>
                        </div>`;
          });
  ...
  addRecentChatting(obj) {
    ...
    const isEdit = $(".edit.recent").hasClass("active");
    const isSelect = $(".selectAll").hasClass("active");
    let initHtml = "";
    if (index != -1) {
      ...
      initHtml = `<div class="Recent_item" data-id="${obj.chat_id}">
        ...
        <div class="checkBox_recent ${isSelect ? "active" : ""}" style="display: ${isEdit ? "flex" : "none"};">
      </div>`;
      $(".RecentChattingBox").prepend(initHtml);
      if(isSelect){
        this.deleteRecArr.push(obj.chat_id);
      }
    }
  ...
  clickFn() {
    ...
    $(".Segment_item").click(function () {
      ...
      if ($(this).attr("id") === "myCharacters") {
        ...
        that.tab = "my";
        history.replaceState(null, "", `/ai-girlfriend.html?my-characters`);
        $(".myCharactersBox").show();
        // 对分类进行排序
        that.composeClassify($(".myCharactersBox"));
      } else if ($(this).attr("id") === "voicepool") {
        that.tab = "voice";
        history.replaceState(null, "", `/ai-girlfriend.html?voice-pool`);
        $(".voicePoolBox,.voicePoolBox_banner").show();
        $(".createBtn").hide();
      } else {
        gtag("event", "click_aigirlfriend_htab");
        history.replaceState(null, "", `/ai-girlfriend.html?home`);
        that.tab = "all";
        $(".SegmentBox").show();
        that.composeRecent();
        // 对分类进行排序
    ...
    $(".edit").click(function () {
      if ($(this).hasClass("active")) {
        ...
        if ($(this).hasClass("recent")) {
          $(".Recent_item .deleteChat,.Recent_item .checkBox_recent").hide(),
            $(".selectAll,.checkBox_recent").removeClass("active"),
            $(".recBox").css("visibility", "hidden"),
            gtag("event", "click_aigirlfriend_hrctedit"),
            that.deleteRecArr = []
            that.isClickSelectAllRec = false
        } else {
        ...
        $(this).hasClass("recent")
          ? ($(".Recent_item .deleteChat").addClass("active").css("display", "flex"),
            $(".Recent_item .checkBox_recent").show(),
            $(".recBox").css("visibility", "visible"),
            gtag("event", "click_aigirlfriend_hrctedit"),
            (that.isClickSelectAllRec = true))
    ...
    $(".selectAll").click(function () {
      that.deleteRecArr = [];
      gtag("event", "selectall_aigirlfriend_hrctdelete");
      $(".selectAll").toggleClass("active");
      if ($(this).hasClass("active")) {
        $(".checkBox_recent").addClass("active");
        that.recentCharacters.forEach((item) => {
          that.deleteRecArr.push(item.chat_id);
        });
      } else {
        $(".checkBox_recent").removeClass("active");
      }
    });

    $(".deleteRec").click(function () {
      if (that.deleteRecArr.length == 0) return;
      gtag("event", "delete_aigirlfriend_hrctdelete");
      const deleteStr = that.deleteRecArr.join(",");

      let deletePop;
      const length = that.deleteRecArr.length;
      let res = length <= 1 ? jsonAiGirlFriend.deleteRecTitle : jsonAiGirlFriend.deleteRecTitleS;
      const title = res.replace("%ss", length);
      deletePop = $Popup({
        title,
        content: jsonAiGirlFriend.deleteRecContent,
        ...
        onClose: () => {
          ...
          fetchPost("chat/user/del-recent-chatting", { chat_id: deleteStr })
            .then(async () => {
            ...
            .catch(function (res) {
              console.error("del-recent-chatting-all", res);
            });
        },
      });
      $(".modal .cannel").click(() => {
    ...
    $(".refersh").click(function () {
      that.initCharacters();
      gtag("event", "refresh_aigirlfriend_hfilter");
    });
    ...
    $("body").on("mouseup", async function (e) {
      ...
      } else if (target.hasClass("checkBox_recent")) {
        // 1.6
        target.toggleClass("active");
        const chat_id = target.parents(".Recent_item").attr("data-id");
        toggleValue(that.deleteRecArr, chat_id);
        that.deleteRecArr.length == that.recentCharacters.length
      ...
      // last
        ...
        console.log(that.isClickSelectAllRec, "isClickSelectAllRec");
        if (that.isClickSelectAllRec) {
          toggleValue(that.deleteRecArr, id);
          target.parents(".Recent_item").children(".checkBox_recent").toggleClass("active");
          that.deleteRecArr.length == that.recentCharacters.length
            ? $(".selectAll").addClass("active")
            : $(".selectAll").removeClass("active");
        } else {
...
function resetPage() {
  ...
  if (ttsBlank(search)) return;
  switch (search) {
    case "chat":
      const storedData = sessionStorage.getItem("repeatData");
      console.log(storedData, "storedData");
      const resetData = storedData ? JSON.parse(storedData) : null;
      if (ttsBlank(resetData) || resetData.path != "chat") return;
      aiChatting.init(resetData.data);
      break;
    case "my-characters":
      $("#myCharacters").click();
      break;
  ...
  // sessionStorage.setItem("repeatData", " ");
  return search == "my-characters";
}
