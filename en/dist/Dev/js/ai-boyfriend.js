...
class AiGirlFriend {
        ...
        setTimeout(async () => {
          if (prop === "mc_Avatar") {
            if (target["mc_Avatar"]) {
              ...
              $(".createMc_container .avatar .addImage span").text(jsonAiBoyFriend.createMc.avatar_btnAdd);
            } else {
              $(".createMc_container .avatar").removeClass("has");
              $(".createMc_container .avatar .addImage span").text(jsonAiBoyFriend.createMc.avatar_btnSet);
            }
          } else if (prop === "mc_Background") {
            if (target["mc_Background"]) {
              $(".createMc_container .background").addClass("has");
              $(".background .addImage span").text(jsonAiBoyFriend.createMc.background_btnAdd);
            } else {
              $(".createMc_container .background").removeClass("has");
              $(".background .addImage span").text(jsonAiBoyFriend.createMc.background_btnSet);
            }
            ...
            let text = "",
              tip =
                value.length > 1 ? jsonAiBoyFriend.createMc.Categories_tips : jsonAiBoyFriend.createMc.Categories_tip;
            ...
            if (value) {
              ...
              if (value === "create") {
                gtag("event", "show_aiboyfriend_create");
                $(".createMc_container").removeClass("editStart");
              } else {
                gtag("event", "show_aiboyfriend_edit");
            ...
            if (target["editUser_Avatar"]) {
              ...
              $(".editUser .avatar .addImage span").text(jsonAiBoyFriend.createMc.avatar_btnAdd);
            } else {
              $(".editUser .avatar").removeClass("has");
              $(".editUser .avatar .addImage span").text(jsonAiBoyFriend.createMc.avatar_btnSet);
            }
          } else if (prop === "editUserAge") {
            if (Number(value) != 0) {
              $(".editUserAge .select_content").text(value).attr("data-age", value);
            } else {
              $(".editUserAge .select_content").text(jsonAiBoyFriend.editUser.age_tip).attr("data-age", 0);
            }
  ...
  data() {
    return {
      ...
      mc_Background: "",
      mc_Persona: "",
      mc_visibility: jsonAiBoyFriend.classifySelect.Public,
      mc_Categories: [],
      mc_Greeting: "",
      mc_Scenario: "",
      mc_Dialogue: "",
      ...
      if (res.code === 200) {
        ...
        data.forEach((item, index) => {
          ...
          let Character_author = item.json.Character_author
            ? jsonAiBoyFriend.by + " " + item.json.Character_author
            : "";
          ...
          <div class="headPortrait">
            ...
            ${
              item.is_self == 1
                ? `<div class="editInbox" >
                <div class="icon"></div>${jsonAiBoyFriend.edit}
              </div>`
                : ""
            }
              <div class="shareInbox">
                <div class="icon"></div>${jsonAiBoyFriend.Share}
              </div>
            </div>
            <div class="chat_info">
          ...
          <div class="character_info">
            ...
            <div class="infoBox">
              <div class="info_l">
                ...
                <div class="like_num">${item.like_num > 1 ? jsonAiBoyFriend.likes : jsonAiBoyFriend.like}</div>
               </div>
              <div class="info_r">
                <button class="chatNow">${jsonAiBoyFriend.chatNow}</button>
              </div>
            </div>
          </div>
          ...
          initHtml += `<div class="myCharacters_item" data-indexId="${item.id}" data-id="${
            ...
            item.like_num
                            ...
                            ${
                              item.status != 4
                                ? `<button class="chatNow">${jsonAiBoyFriend.chatNow}</button>`
                                : `<button class="editDraft"><div class="icon"></div>${jsonAiBoyFriend.edit}</button>`
                            }
                            </div>
                          </div>
                        </div>
    ...
    try {
      ...
      if (res.code === 200) {
        ...
        if (data.length > 0) {
          ...
          $(".edit.recent").removeClass("active").text(jsonAiBoyFriend.edit);
        }
      }
    } catch (err) {
  ...
  async GetClassify(data) {
    if (data.length > 0) {
      this.classifys = data;
      let initHtml = `<div class="classify_item active" data-tag=''>${jsonAiBoyFriend.AllCharacters}</div>`;
      let categoriesHtml = "";
      data.forEach((item) => {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          if (tryTimes <= 3) {
            ...
            $Popup({
              title: jsonAiBoyFriend.failedMsg.normalTitle,
              content: jsonAiBoyFriend.failedMsg.generateError,
              type: "error",
            });
            return;
          }
  ...
  async GetRoleInfo(id, is_my_characters = 2, my_characters_id = 0) {
    try {
      ...
      if (res.code === 200) {
        ...
        this.mc_visibility =
          data.type == 1 ? jsonAiBoyFriend.classifySelect.Public : jsonAiBoyFriend.classifySelect.Private;
  ...
  async roleEdit(successBack = () => {}, errorBack = () => {}) {
    try {
      let type;
      if (this.mc_visibility == jsonAiBoyFriend.classifySelect.Public) {
        type = 1;
        gtag("event", "public_aiboyfriend_edit");
      } else {
        type = 2;
        gtag("event", "private_aiboyfriend_edit");
      }
      ...
      if (res.code == 401) {
        gtag("event", "alert_aiboyfriend_loginwinedit");
        let bootLogin = $Popup({
          title: jsonAiBoyFriend.createMc.bootLoginTitle,
          content: jsonAiBoyFriend.createMc.bootLoginDesc,
          closeBtn: jsonAiBoyFriend.login,
          applyBtn: jsonAiBoyFriend.createMc.NotNow,
          autoClose: false,
          exist: "bootLogin",
          onClose: () => {
            gtag("event", "login_aiboyfriend_loginwinedit");
            bootLogin.close();
            showLoginWindow({
              fn: () => {
                gtag("evet", "succ_aiboyfriend_loginwinedit");
              },
            });
          },
          onApply: () => {
            gtag("event", "notnow_aiboyfriend_loginwindit");
            bootLogin.close();
            this.closeCD();
            this.clearInput();
            this.editMcType = "";
          ...
          topCloseFn: () => {
            gtag("event", "close_aiboyfriend_loginwinedit");
          },
        });
        errorBack?.();
        ...
        if (this.is_draft) {
          $Popup({
            title: jsonAiBoyFriend.failedMsg.normalTitle,
            content: jsonAiBoyFriend.failedMsg.draftSaveError,
            type: "error",
          });
        } else {
          $Popup({
            title: jsonAiBoyFriend.failedMsg.normalTitle,
            content: jsonAiBoyFriend.failedMsg.publishError,
            type: "error",
          });
        }
        errorBack?.();
  ...
  clickFn() {
    ...
    $(".heart").click(function () {
      ...
      $(this).hasClass("active")
        ? (that.isLike = false)
        : ((that.isLike = true), gtag("event", "like_aiboyfriend_hfilter"));
    ...
    $(".Segment_item").click(function () {
      ...
      if ($(this).attr("id") === "myCharacters") {
        gtag("event", "click_aiboyfriend_mytab");
        if (!that.isInitMyCharacters) {
          that.GetMyCharacters();
          that.isInitMyCharacters = true;
        }
        that.tab = "my";
        ...
        gtag("event", "click_aiboyfriend_htab");
        that.tab = "all";
        $(".SegmentBox").show();
    ...
    $(".edit").click(function () {
      if ($(this).hasClass("active")) {
        ...
        $(this).text(jsonAiBoyFriend.edit);
        $(this).hasClass("recent")
          ? ($(".Recent_item .deleteChat").removeClass("active").hide(), gtag("event", "click_aiboyfriend_hrctedit"))
          : ($(".myCharacters_item .deleteChat").removeClass("active").hide(),
            gtag("event", "click_aiboyfriend_mycedit"),
            $(".myCharacters_item .share_icon").css("display", "flex"));
      } else {
        $(this).addClass("active");
        $(this).text(jsonAiBoyFriend.Done);
        $(this).hasClass("recent")
          ? ($(".Recent_item .deleteChat").addClass("active").css("display", "flex"),
            gtag("event", "click_aiboyfriend_hrctedit"))
    ...
    $(".createMc").click(function () {
      ...
      if ($(this).hasClass("createBtn")) {
        gtag("event", "click_aiboyfriend_create");
      } else {
        gtag("event", "click_aiboyfriend_nodatacreate");
      }
    });
    ...
    $(".create_back_box,.mc_btn.discard").click(function () {
      if (that.editMcType == "edit") {
        if ($(this).hasClass("discard")) {
          gtag("event", "discard_aiboyfriend_edit");
        } else {
          gtag("event", "back_aiboyfriend_edit");
        }
        ...
        if (!getCookie("access_token")) {
          ...
          if (isSame) {
            ...
            const toSave = $Popup({
              title: jsonAiBoyFriend.toSaveTitle,
              content: jsonAiBoyFriend.toSaveDesc,
              closeBtn: jsonAiBoyFriend.save,
              applyBtn: jsonAiBoyFriend.createMc.NotNow,
              autoClose: false,
              exist: "bootLogin",
              onClose: () => {
    ...
    $("#createMc").click(async function () {
      let isPass = true;
      if (!that.mc_Avatar) {
        $(".erorrTip[data-type='avatar']").find(".image_error").text(jsonAiBoyFriend.createMc.avatar_not);
      ...
      if (!isPass) return;
      gtag("event", "click_aiboyfriend_aicreate");
    ...
    $("#editMc").click(async function () {
      gtag("event", "publish_aiboyfriend_edit");
    ...
    $("#draftMc").click(async function () {
      gtag("event", "draft_aiboyfriend_edit");
      if (!getCookie("access_token")) {
        editShowLogin();
        return;
      }
    ...
    $(".writeMe").click(function () {
      gtag("event", "show_aiboyfriend_aiwrite");
      let createPop = $Popup({
        type: "createMC",
        title: jsonAiBoyFriend.createMc.writeMe_title,
        content: $(".whiteMeContent").html(),
        closeBtn: jsonAiBoyFriend.createMc.Generate,
        applyBtn: jsonAiBoyFriend.createMc.Apply,
        autoClose: false,
        exist: "exist",
        onClose: () => {
          gtag("event", "generate_aiboyfriend_aiwrite");
        ...
        onApply: () => {
          gtag("event", "apply_aiboyfriend_aiwrite");
    ...
    $(".header_editUser").click(() => {
      gtag("event", "editprofile_aiboyfriend_header");
      that.clearUserData();
      let createPop = $Popup({
        type: "editUser",
        title: jsonAiBoyFriend.editUser.title,
        content: $(".editUserInfo").html(),
        closeBtn: jsonAiBoyFriend.editUser.save,
        applyBtn: jsonAiBoyFriend.editUser.cancel,
        autoClose: false,
        exist: "editUser",
        onClose: () => {
          let isPass = true;
          if (!that.editUser_Avatar) {
            $(".erorrTip[data-type='editUserAvatar']").find(".image_error").text(jsonAiBoyFriend.createMc.avatar_not);
      ...
      if (target.hasClass("like_icon")) {
        ...
        if (target.parents(".characters_item").length != 0) {
          ...
          if (target.hasClass("active")) {
            ...
            target.siblings(".like_num").text(num > 1 ? jsonAiBoyFriend.likes : jsonAiBoyFriend.like);
            ...
            target.siblings(".like_num").text(num > 1 ? jsonAiBoyFriend.likes : jsonAiBoyFriend.like);
            target.data("likes", num);
            gtag("event", "click_aiboyfriend_hlikebtn");
            that.charactersList[resIndex].is_like = 1;
          }
        } else {
          if (!target.hasClass("active")) {
            gtag("event", "click_aiboyfriend_myclikebtn");
        ...
        if (target.parents(".characters_item").length != 0) {
          gtag("event", "click_aiboyfriend_hedit");
          const res = that.charactersList[resIndex];
          await that.GetRoleInfo(res.role_id, 1, res.my_chat_id);
        } else {
          gtag("event", "click_aiboyfriend_mycchatedit");
          const res = that.mycharactersList[resIndex];
        ...
        if (ttsBlank(tag)) {
          ...
          if (isHome) {
            ...
            if (index !== -1) {
              ...
              gtag("event", `htag_aiboyfriend_${tagEve}`);
            }
            ...
            if (index !== -1) {
              ...
              let tagEve = target.attr("data-tag");
              tagEve = tagEve.replace(/\s+/g, "").toLowerCase();
              gtag("event", `mytag_aiboyfriend_${tagEve}`);
            }
          ...
          let text = "",
            tip =
              that.mc_Categories.length > 1
                ? jsonAiBoyFriend.createMc.Categories_tips
                : jsonAiBoyFriend.createMc.Categories_tip;
        ...
        if (target.parents(".deleteChat").hasClass("recent")) {
          gtag("event", "click_aiboyfriend_hrctdelete");
          const chat_id = target.parents(".Recent_item").attr("data-id");
          deletePop = $Popup({
            title: jsonAiBoyFriend.deleteRecentTitle,
            content: jsonAiBoyFriend.deleteRecentText,
            closeBtn: jsonAiBoyFriend.Delete,
            otherBtns: `<button class="cannel">${jsonAiBoyFriend.Cancel}</button>`,
            autoClose: false,
            exist: "deletePop",
            onClose: () => {
              gtag("event", "del_aiboyfriend_hrctdelete");
          ...
          $(".modal .cannel").click(() => {
            gtag("event", "cancel_aiboyfriend_hrctdelete");
            deletePop.close();
          });
        } else {
          gtag("event", "click_aiboyfriend_mycdelete");
          const my_characters_id = that.mycharactersList[resIndex].my_characters_id;
          deletePop = $Popup({
            title: jsonAiBoyFriend.deleteMyChatTitle,
            content: jsonAiBoyFriend.deleteMyChatText,
            closeBtn: jsonAiBoyFriend.Delete,
            otherBtns: `<button class="cannel">${jsonAiBoyFriend.Cancel}</button>`,
            autoClose: false,
            exist: "deletePop",
            onClose: () => {
              gtag("event", "del_aiboyfriend_mycdelete");
              deletePop.loading.start();
          ...
          $(".modal .cannel").click(() => {
            gtag("event", "cancel_aiboyfriend_mycdelete");
            deletePop.close();
          });
        }
      } else if (target.parents(".characters_item").length != 0) {
        if (target.hasClass("chatNow")) {
          gtag("event", "click_aiboyfriend_hchatbtn");
        } else {
          gtag("event", "click_aiboyfriend_hcard");
        }
        //点击charactrers
        ...
        //点击myCharacters
        if (target.hasClass("chatNow")) {
          gtag("event", "click_aiboyfriend_mycchatbtn");
        } else {
          gtag("event", "click_aiboyfriend_myccard");
        }
        // 如果选中了删除模式 return
        ...
        if (status != 4) {
          ...
          // is Draft
          gtag("event", "click_aiboyfriend_myccardedit");
          that.editMcType = "edit";
        ...
        gtag("event", "click_aiboyfriend_hrctcard");
        //点击最近聊天
        ...
        if (target.parents(".editUserAge").length != 0) {
          ...
          switch (age) {
            case "1-10":
              gtag("event", "click_aiboyfriend_agerange1");
              break;
            case "10-15":
              gtag("event", "click_aiboyfriend_agerange2");
              break;
            case "15-20":
              gtag("event", "click_aiboyfriend_agerange3");
              break;
            case "20-25":
              gtag("event", "click_aiboyfriend_agerange4");
              break;
            case "25-30":
              gtag("event", "click_aiboyfriend_agerange5");
              break;
            case "30-40":
              gtag("event", "click_aiboyfriend_agerange6");
              break;
            case "40-50":
              gtag("event", "click_aiboyfriend_agerange7");
              break;
            case "50+":
              gtag("event", "click_aiboyfriend_agerange8");
              break;
          }
        }
        ...
        if (target.parents(".classify").length != 0) {
          ...
          switch (primary) {
            case "":
              gtag("event", "all_aiboyfriend_filter");
              break;
            case jsonAiBoyFriend.classifySelect.Public:
              gtag("event", "public_aiboyfriend_filter");
              break;
            case jsonAiBoyFriend.classifySelect.Private:
              gtag("event", "private_aiboyfriend_filter");
              break;
            case jsonAiBoyFriend.classifySelect.Draft:
              gtag("event", "draft_aiboyfriend_filter");
              break;
          }
        }
        // 打点
        if (Object.keys(target.data())[0] === "sort" && target.parents(".character_select").length != 0) {
          ...
          switch (true) {
            case sort == "top":
              gtag("event", "top_aiboyfriend_hfilter");
              break;
            case sort == "hot":
              gtag("event", "hot_aiboyfriend_hfilter");
              break;
          }
        } else {
          switch (true) {
            case sort_time == "day":
              gtag("event", "top_aiboyfriend_hdayfilter");
              break;
            case sort_time == "week":
              gtag("event", "top_aiboyfriend_hweekfilter");
              break;
            case sort_time == "month":
              gtag("event", "top_aiboyfriend_hmonthfilter");
              break;
            case sort_time == "year":
              gtag("event", "top_aiboyfriend_hyearfilter");
              break;
            case sort_time == "":
              gtag("event", "top_aiboyfriend_halltimefilter");
              break;
          }
        }
    ...
    $(".view,.viewMore,.back").click(function () {
      ...
      if ($(this).hasClass("active")) {
        $(".view").text(jsonAiBoyFriend.view);
        $(this).removeClass("active");
        $(".RecentChatting").removeClass("viewAll");
        that.composeRecent();
        $(".viewMore").removeClass("active");
      } else {
        $(".view").text(jsonAiBoyFriend.viewLess);
        $(this).addClass("active");
    ...
    await this.roleEdit(
      () => {
        ...
        if (type == "all") {
          ...
          $(".myCharactersBox .classify_select .select_content").text(jsonAiBoyFriend.classifySelect.All);
          this.primary = "";
        } else if (type == "draft") {
          $(".myCharactersBox .classify_select .select_list_item").removeClass("active");
          $(".myCharactersBox .classify_select .select_list_item").eq(3).addClass("active");
          $(".myCharactersBox .classify_select .select_content").text(jsonAiBoyFriend.classifySelect.Draft);
          this.primary = jsonAiBoyFriend.classifySelect.Draft;
        }
  ...
  //分享弹窗与处理
  showShare(id, value) {
    gtag("event", "share_aiboyfriend_winshow");
    $Popup({
    ...
    $(".shareIco.Link").click(() => {
      gtag("event", "share_aiboyfriend_link");
  ...
  async uploadImage(parent, e, that, type, callback) {
    ...
    if (!checkFileType(file)) {
      parent.siblings(".erorrTip").find(".image_error").text(jsonAiBoyFriend.imageNoSupported);
    ...
    if (file?.size > 100 * 1024 * 1024) {
      parent.siblings(".erorrTip").find(".image_error").text(jsonAiBoyFriend.imageSizeError);
      parent.siblings(".erorrTip").css("visibility", "visible");
      $(that).val(null);
      callback?.();
      return false;
    ...
    img.onload = function () {
      ...
      if (type === "background") {
        if (height < 128 || width < 128) {
          parent.siblings(".erorrTip").find(".image_error").text(jsonAiBoyFriend.backGround_Error);
          parent.siblings(".erorrTip").css("visibility", "visible");
          return false;
        } else {
        ...
        if (height < 32 || width < 32) {
          parent.siblings(".erorrTip").find(".image_error").text(jsonAiBoyFriend.avatar_error);
          parent.siblings(".erorrTip").css("visibility", "visible");
          return false;
        } else {
          if (type == "avatar") {
  ...
  clearInput() {
    ...
    this.mc_Background = "";
    this.mc_Persona = "";
    this.mc_visibility = jsonAiBoyFriend.classifySelect.Public;
    this.mc_Categories = [];
    this.mc_Greeting = "";
    this.mc_Scenario = "";
    this.mc_Dialogue = "";
      ...
      await fetchPostNormal("file-upload", formData, interHost, {})
        .then((res) => {
          if (res.code != 200) {
            ...
            $Popup({
              title: jsonAiBoyFriend.failedMsg.normalTitle,
              content: jsonAiBoyFriend.errorDesc,
              type: "error",
            });
          } else {
...
$(document).ready(function () {
  ...
  let fun = async () => {
    ...
    if (!access_token) {
      gtag("event", "open_aiboyfriend_page");
    }
