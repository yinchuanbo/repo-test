...
function jqAjaxPromise(param, progressFun = () => { }, origin = "web") {
  ...
  request_header['Request-Language'] = 'fr';
...
const getinsurValue = () => {
  ...
  if (url == "/face-swap.html") {
    insurValue = "fr_vidq_faceswap";
  } else if (url == "/video-translator.html") {
    insurValue = "fr_vidq_videotranslate";
  } else if (url == "/") {
    // insurValue = "fr_home";
  } else if (url == "/about.html") {
    // insurValue = "fr_about";
  } else if (url == "/contact.html") {
    // insurValue = "fr_contact";
  } else if (url == "/privacy.html") {
    insurValue = "fr_privacy";
  } else if (url == "/terms.html") {
    insurValue = "fr_terms";
  } else if (url == "/ai-attractiveness-test.html") {
    insurValue = "fr_vidq_attracttest";
  }else if (url == "/multiple-face-swap.html") {
    insurValue = "fr_vidq_mulfaceswap";
  }

  if (isSeo()) {
    insurValue = insurValue.replace("fr", "frseo");
  }
  return insurValue;
};
  ...
  fetchGet("ai/public/get-country-type")
    .then((res) => {
      ...
      if ((countryType === "T1" || countryType === "T2") && judgeClient() === 'ios' && document.location.pathname !== '/pricing.html') {
        $('.fix-footer').css("bottom", '1.2rem');
        $("#iosFooter").show()
        $("#ios_footer_btn").click(function () {
          gtag("event", "fr_faceappclick_fcios");
          window.location.href = "https://apps.apple.com/app/apple-store/id6497986083?pt=127019031&ct=from_fr_vidq&mt=8";
        });
      }
    }) 
    .catch((err) => {
