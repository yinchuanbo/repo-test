...
function getCreditsTextCreditheader(name, valData = {}, bool = false) { 
  if (bool) {
      let num = valData.val;
...
class HeaderCredit extends HTMLElement {
  ...
  setPopoverNode() {
    ...
    if (this.is_credit_subscription != 1) {
      item1Node.html(getCreditsTextCreditheader("photos_num", { val: 960 }));
      item2Node.html(getCreditsTextCreditheader("videos_mins", { val: 48 }));
      nodeBtn.text(getCreditsTextCreditheader("Get_Now"))
    } else {
      nodeBtn.text(getCreditsTextCreditheader("More_credits"))
      item1Node.html(getCreditsTextCreditheader("photos_num", { val: 1500 }));
      item2Node.html(getCreditsTextCreditheader("videos_mins", { val: 75 }));
    }
  }
  ...
  updateNode(){
    $(this).find('.credit_num').text(getCreditsTextCreditheader('credit',{val: this.credits},true));
    $(this).find('.pop_title').text(getCreditsTextCreditheader('video_translate_title'));
    $(this).find('.pop_span').text(getCreditsTextCreditheader('video_translate_span'));
    $(this).find('.get_credit_title').text(getCreditsTextCreditheader('Get_credits_for'));
    $(this).find('.get_now').text(getCreditsTextCreditheader('Get_Now'));
    ...
    if(isMobileDevice()){
      ...
      $('#m_login_user').html(
        `
          ...
          <div>${getCreditsTextCreditheader('credit',{val: ""})}: ${this.credits}</div>
        `
      )
      if (getCookie("access_token")) {
        $('#m_login_user').show();
