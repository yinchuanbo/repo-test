...
function jqAjaxPromise(param, progressFun = () => { }, origin = "web") {
  ...
  request_header['Request-Language'] = 'ar';
...
const getinsurValue = () => {
  ...
  if (url == "/face-swap.html") {
    insurValue = "ar_vidq_faceswap";
  } else if (url == "/video-translator.html") {
    insurValue = "ar_vidq_videotranslate";
  } else if (url == "/") {
    // insurValue = "ar_home";
  } else if (url == "/about.html") {
    // insurValue = "ar_about";
  } else if (url == "/contact.html") {
    // insurValue = "ar_contact";
  } else if (url == "/privacy.html") {
    insurValue = "ar_privacy";
  } else if (url == "/terms.html") {
    insurValue = "ar_terms";
  } else if (url == "/ai-attractiveness-test.html") {
    insurValue = "en_vidq_attracttest";
  }else if (url == "/multiple-face-swap.html") {
    insurValue = "en_vidq_mulfaceswap";
  }

  if (isSeo()) {
    insurValue = insurValue.replace("ar", "arseo");
  }
  return insurValue;
};
    ...
    .then((res) => {
      ...
      if ((countryType === "T1" || countryType === "T2") && judgeClient() === 'ios' && document.location.pathname !== '/pricing.html') {
        ...
        $("#ios_footer_btn").click(function () {
          gtag("event", "ar_faceappclick_fcios");
          window.location.href = "https://apps.apple.com/app/apple-store/id6497986083?pt=127019031&ct=from_ar_vidq&mt=8";
        });
      }
    }) 
    .catch((err) => {
