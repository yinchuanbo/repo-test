...
class Attractiveness {
  ...
  setFirstStart = (flag) => {
    if (flag) {
      ...
      // $(".show2").hide();
      // $(".multi_people_list_loading").show();
      ...
      // $(".show2").hide();
      // $(".multi_people_list_loading").hide();
  ...
  setEvaluates() {
    ...
    console.log(evaluates, maxItem)
    if (len > 1) {
      randomtext = this.getRandomNum(1, 50); // 随即图标
    }
    console.log(max, list, len, maxItem);
  ...
  getPublicOptions() {
    const that = this;
    fetchGet("ai/public/options?type=ai-attractiveness-test&country=es").then(res => {
